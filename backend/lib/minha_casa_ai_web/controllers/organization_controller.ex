defmodule MinhaCasaAiWeb.OrganizationController do
  ...
  def create(conn, params) do
    case Organizations.create(current_user_id(conn), params) do
      {:ok, organization} ->
        conn |> put_status(:created) |> json(%{organization: OrganizationJSON.organization(organization)})
  ...
  def show(conn, %{"id" => id}) do
    case Organizations.get_for_user(id, current_user_id(conn)) do
      {:ok, organization} -> json(conn, %{organization: OrganizationJSON.organization(organization)})
  ...
  def add_member(conn, %{"id" => id} = params) do
    ...
    with {:ok, %Organization{} = organization} <- Organizations.get_for_user(id, user_id),
         ...
         :ok <- can_assign_role?(Map.get(organization, :role), role),
         {:ok, member} <- Organizations.add_member(id, email, role) do
      conn |> put_status(:created) |> json(%{member: OrganizationJSON.member(member)})
    else
      {:error, :not_found} -> not_found(conn, "Organization")
      {:error, :invalid_role} -> conn |> put_status(:bad_request) |> json(%{error: "Invalid role"})
      {:error, :user_not_found} -> conn |> put_status(:not_found) |> json(%{error: "User not found with this email"})
      {:error, :already_member} -> conn |> put_status(:bad_request) |> json(%{error: "User is already a member"})
      {:error, :forbidden_role} -> forbidden(conn, "Only owners can assign owner role")
      false -> forbidden(conn, "Only owners and admins can add members")
      {:error, changeset} -> changeset_error(conn, changeset)
    end
  end
  ...
  def update_member(conn, %{"id" => id, "user_id" => target_user_id, "role" => role}) do
    ...
    with {:ok, %Organization{} = organization} <- Organizations.get_for_user(id, user_id),
      ...
      {:error, :not_found} -> not_found(conn, "Organization")
      nil -> not_found(conn, "Member")
      {:error, :invalid_role} -> conn |> put_status(:bad_request) |> json(%{error: "Invalid role"})
      {:error, :forbidden_role} -> forbidden(conn, "Only owners can change owner roles")
      {:error, :last_owner} -> conn |> put_status(:bad_request) |> json(%{error: "Cannot demote the last owner"})
      false -> forbidden(conn, "Only owners and admins can update member roles")
      {:error, changeset} -> changeset_error(conn, changeset)
    end
  end
  ...
  def remove_member(conn, %{"id" => id, "user_id" => target_user_id}) do
    ...
    with {:ok, %Organization{} = organization} <- Organizations.get_for_user(id, user_id),
      ...
      {:error, :not_found} -> not_found(conn, "Organization")
      nil -> not_found(conn, "Member")
      {:error, :forbidden_role} -> forbidden(conn, "Only owners can remove owners")
      {:error, :last_owner} -> conn |> put_status(:bad_request) |> json(%{error: "Cannot remove the last owner"})
      false -> forbidden(conn, "Only owners and admins can remove members")
      {:error, changeset} -> changeset_error(conn, changeset)
    end
  end

  defp can_assign_role?("owner", role) when role in ["owner", "admin", "member"], do: :ok
  defp can_assign_role?(_, "owner"), do: {:error, :forbidden_role}
  defp can_assign_role?(_, role) when role in ["admin", "member"], do: :ok
  defp can_assign_role?(_, _), do: {:error, :invalid_role}

  defp can_change_role?("owner", _current, role) when role in ["owner", "admin", "member"], do: :ok
