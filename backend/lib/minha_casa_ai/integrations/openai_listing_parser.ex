defmodule MinhaCasaAi.Integrations.OpenAIListingParser do
  ...
  def parse_text(raw_text, opts \\ []) when is_binary(raw_text) do
    catalog = Keyword.get(opts, :catalog, ListingFeatures.default_system_options())
  ...
  defp prompt_compile_vars(catalog) do
    %{
      ...
      "feature_list" => ListingFeatures.feature_list_for_prompt(catalog)
    }
  end
  ...
  defp valid_listing?(listing) when is_map(listing) do
    present?(listing["address"]) || positive_number?(listing["price"])
  end
  ...
  def build_listing(parsed, catalog) do
    features = normalize_parsed_features(parsed, catalog)

    %{
      "title" => "",
      "address" => parsed["address"] || "Endereço não informado",
      "neighborhood" => parsed["neighborhood"],
      "city" => parsed["city"],
      "totalAreaM2" => parsed["totalAreaM2"],
      "privateAreaM2" => parsed["privateAreaM2"],
      ...
      "constructionYear" => ConstructionYear.normalize(parsed["constructionYear"])
    }
  end

  defp normalize_parsed_features(parsed, catalog) do
    raw = Map.get(parsed, "features") || %{}
