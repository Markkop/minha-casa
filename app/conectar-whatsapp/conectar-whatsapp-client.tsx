...
import {
  clearPendingWaLinkCode,
  resolveWaLinkCode,
  savePendingWaLinkCode,
} from "@/lib/whatsapp-link-pending"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"

type LinkState = "idle" | "linking" | "success" | "error"

export function ConectarWhatsAppClient() {
  ...
  const urlCode = searchParams.get("wa")
  const code = resolveWaLinkCode(urlCode)

  const redirectPath = code
    ? `/conectar-whatsapp?wa=${encodeURIComponent(code)}`
    : "/conectar-whatsapp"

  useEffect(() => {
    if (code) {
      savePendingWaLinkCode(code)
    }
  }, [code])
      ...
      if (!response.ok) {
        ...
        setError(
          typeof payload.error === "string"
            ? payload.error
            : "Não foi possível conectar o WhatsApp. Tente gerar um novo código no chat."
        )
        return
      }
      ...
      <div className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center px-4">
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>
              Envie qualquer mensagem para o bot no WhatsApp para receber um link com código de
              conexão.
            </CardDescription>
          </CardHeader>
        </Card>
      ...
      <div className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center px-4">
        <Card className="w-full max-w-md">
          <CardHeader>
            <CardTitle>Conectar WhatsApp</CardTitle>
            <CardDescription>
              Entre ou crie sua conta para vincular o WhatsApp ao Minha Casa. Seu código{" "}
      ...
      <div className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center px-4">
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>
              Sua conta foi vinculada. Volte ao WhatsApp e envie anúncios, links ou arquivos para
              análise.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <CardHeader>
          ...
          <CardDescription>
            {linkState === "linking"
              ...
              : "Confirme a conexão do seu WhatsApp com o Minha Casa."}
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-3">
