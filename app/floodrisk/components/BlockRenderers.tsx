...
export const StreetSurface = ({ data }: { data: ElementHeight }) => {
  ...
  return (
    <group>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} receiveShadow>
        <planeGeometry args={[width, depth]} />
        <meshStandardMaterial color={floodSceneColors.street} roughness={0.6} /> {/* Slate 700 */}
      </mesh>
      ...
      <group position={[0, 0.02, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <mesh position={[-width / 2 + 0.2, 0, 0]}>
           ...
           <meshBasicMaterial color={floodSceneColors.line} />
        </mesh>
      ...
      {showModel && (
        <group position={[0, 1, -1]}>
           ...
           <mesh position={[0, 0, 0]} castShadow>
              ...
              <meshStandardMaterial color={floodSceneColors.houseWall} />
           </mesh>
           {/* Roof - made bigger to cover corners */}
           <mesh position={[0, 1.5, 0]} rotation={[0, Math.PI / 4, 0]} castShadow>
              <coneGeometry args={[4.2, 1.5, 4]} />
              <meshStandardMaterial color={floodSceneColors.houseRoof} />
           </mesh>
           {/* Door */}
           <mesh position={[0, -0.5, 2.01]}>
               <planeGeometry args={[1, 1.5]} />
               <meshStandardMaterial color={floodSceneColors.door} />
           </mesh>
           {/* Window */}
           <mesh position={[1.2, 0.2, 2.01]}>
               <planeGeometry args={[0.8, 0.8]} />
               <meshStandardMaterial color={floodSceneColors.glass} />
           </mesh>
        </group>
      )}
    </group>
...
export const GarageSurface = ({ data, showModel = true }: { data: ElementHeight; showModel?: boolean }) => {
  return (
    <group>
      {/* Concrete floor */}
      <BaseSurface width={data.width} depth={data.depth} color={floodSceneColors.garage} />
      ...
      {showModel && (
        <group position={[0, 0.4, 0]} rotation={[0, Math.PI / 2, 0]}>
           <mesh position={[0, 0, 0]} castShadow>
              ...
              <meshStandardMaterial color={floodSceneColors.car} />
           </mesh>
           ...
           <mesh position={[1, -0.2, 1]} rotation={[Math.PI/2,0,0]}>
               ...
               <meshStandardMaterial color={floodSceneColors.wheel} />
           </mesh>
           <mesh position={[-1, -0.2, 1]} rotation={[Math.PI/2,0,0]}>
               <cylinderGeometry args={[0.3, 0.3, 0.2, 16]} />
               <meshStandardMaterial color={floodSceneColors.wheel} />
    ...
    <group>
      ...
      <group position={[-1, 0, -2]}>
          <mesh position={[0, 0.5, 0]} castShadow>
              ...
              <meshStandardMaterial color={floodSceneColors.shrub} />
          </mesh>
          <mesh position={[0, 0, 0]}>
              <cylinderGeometry args={[0.1, 0.1, 0.4]} />
              <meshStandardMaterial color={floodSceneColors.wood} />
          </mesh>
      </group>
      ...
      <group position={[1.2, 0, 3]}>
          <mesh position={[0, 0.4, 0]} castShadow>
              ...
              <meshStandardMaterial color={floodSceneColors.houseGround} />
          </mesh>
          <mesh position={[0, 0, 0]}>
      ...
      <mesh position={[0, 0.01, 0]} rotation={[-Math.PI/2, 0, 0]}>
         ...
         <meshStandardMaterial color={floodSceneColors.stone} />
      </mesh>
