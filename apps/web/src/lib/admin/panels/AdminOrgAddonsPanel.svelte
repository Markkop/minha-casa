...
<section class="rounded-md border border-app-border bg-app-surface">
  <div class="border-b border-app-border p-4">
    ...
    <p class="text-sm text-app-muted">Consulte os workspaces coletivos e suas capacidades concedidas.</p>
  </div>
  <div class="overflow-x-auto">
    <table class="w-full min-w-[920px] text-left text-sm">
      <thead class="bg-app-surface-muted text-xs uppercase text-app-muted">
        <tr><th class="px-3 py-3">Família ou imobiliária</th><th class="px-3 py-3">Tipo</th><th class="px-3 py-3">Owner</th><th class="px-3 py-3">Workspace / seats</th><th class="px-3 py-3">Capacidades</th><th class="px-3 py-3">Ações</th></tr>
      </thead>
      <tbody>
        {#each admin.organizations as org (org.id)}
          <tr class="border-t border-app-border">
            ...
            <td class="px-3 py-3"><Button class="h-8 px-3" variant="secondary" onclick={() => void admin.openOrgAddons(org)}>Gerenciar capacidades</Button></td>
          </tr>
        {/each}
      </tbody>
    </table>
...
{#if admin.mode === "org-addons"}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Capacidades da família ou imobiliária"
      tabindex="-1"
      onkeydown={admin.handleModalKeydown}
    >
      <div class="mb-4 flex items-start justify-between gap-3">
        <div>
          <h2 class="text-lg font-semibold">Capacidades do workspace</h2>
