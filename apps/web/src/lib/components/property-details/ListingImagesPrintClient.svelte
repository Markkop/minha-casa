<script lang="ts">
  ...
  import ListingImagesPrintPicker from "$lib/components/property-details/ListingImagesPrintPicker.svelte";
  ...
  let listing = $state<Property | null>(null);
  ...
  const backHref = $derived(buildPropertyHref(listingId, { tab: "imagens" }));
  ...
  $effect(() => {
    ...
    if (!listingId) {
      loadError = "Informe o imóvel para imprimir as imagens.";
      isLoading = false;
      return;
    }
