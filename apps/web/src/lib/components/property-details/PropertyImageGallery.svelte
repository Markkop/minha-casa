<script lang="ts">
  ...
  import { useImageLightbox } from "$lib/components/property-details/use-image-lightbox.svelte";
  import ListingImageCarousel from "$lib/carousel/ListingImageCarousel.svelte";
  import { buildListingImagesPrintHref } from "$lib/property-details-url";
  ...
  const coverIndex = $derived(normalizeCoverIndex(listing.coverImageIndex, imageUrls.length));
  const carouselUrls = $derived(galleryImages.map((image) => image.url));
  const printHref = $derived(buildListingImagesPrintHref(listing.id));
  ...
  async function persistListingUpdate(updates: Partial<Property>) {
  ...
  function handleSetCover() {
    ...
    void persistListingUpdate({ coverImageIndex: selectedImage.originalIndex });
  }
