<script lang="ts">
  import { WORKSPACE_CONTENT_CLASS, WORKSPACE_STACK_CLASS } from "$lib/workspace-chrome";

  let {
    title,
    description,
    status,
    children
  } = $props<{
    title?: string;
    description?: string;
    status?: string;
    children?: import("svelte").Snippet;
  }>();
</script>

<svelte:head>
  {#if title}
    <title>{title} | Prisma</title>
  {/if}
</svelte:head>

<main class="app-page-background min-h-[calc(100vh-var(--nav-height,2.75rem))] text-app-fg">
  <div class={WORKSPACE_CONTENT_CLASS}>
    <div class={WORKSPACE_STACK_CLASS}>
      {#if title || description || status}
        <div class="app-panel-surface rounded-lg border border-app-border px-4 py-3">
          {#if status}
            <div class="font-mono text-[0.6875rem] font-medium uppercase tracking-[0.16em] text-app-cyan">{status}</div>
          {/if}
          {#if title}
            <h1 class="app-page-title text-xl font-semibold text-app-fg">{title}</h1>
          {/if}
          {#if description}
            <p class="mt-1 max-w-[65ch] text-sm leading-6 text-app-muted">{description}</p>
          {/if}
        </div>
      {/if}

      {@render children?.()}
    </div>
  </div>
</main>
