<script lang="ts">
  ...
  import { formatCurrency, type GeocodedListing } from "$lib/anuncios/map-shared";
  ...
  let {
    ...
    getTitle = (listing: Imovel) => listing.titulo || listing.endereco || "Imóvel"
  } = $props<{
    geocodedListings: GeocodedListing[];
    collectionLabel?: string;
    getTitle?: (listing: Imovel) => string;
  }>();
  ...
  const markers = $derived.by((): PropertyMarker[] =>
    visibleListings.map((item: LocatedCollectionListing, index: number) => ({
      ...
      price: item.listing.preco != null ? compactPrice(item.listing.preco) : undefined,
      status: item.listing.listingEtapa || undefined,
      position: item.location,
      detail: [item.listing.bairro, item.listing.cidade].filter(Boolean).join(", ") || undefined
    }))
  );
  ...
  function payloadForContext(
    ...
    const neighborhood = focus.bairro?.trim() ?? "";
    const city = focus.cidade?.trim() ?? "";
    return {
      ...base,
      center,
      place: {
  ...
  function knownArea(listing: Imovel): string | null {
    const area = listing.m2Privado ?? listing.m2Totais;
    return area && area > 0 ? `${new Intl.NumberFormat("pt-BR").format(area)} m²` : null;
  }

  function knownLocation(listing: Imovel): string | null {
    return [listing.endereco, listing.bairro, listing.cidade].filter(Boolean).join(" · ") || null;
  }
</script>
  ...
  {#if selectedListing}
    <article class="absolute bottom-4 left-3 z-30 w-[min(22rem,calc(100%-1.5rem))] rounded-2xl border border-white/12 bg-[#09142f]/94 p-4 shadow-2xl backdrop-blur-2xl sm:left-4">
      ...
      <div class="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs">
        {#if selectedListing.preco != null}<strong>{formatCurrency(selectedListing.preco)}</strong>{/if}
        {#if knownArea(selectedListing)}<span class="text-slate-300">{knownArea(selectedListing)}</span>{/if}
        {#if selectedListing.listingEtapa}<span class="text-slate-300">{selectedListing.listingEtapa}</span>{/if}
      </div>
    </article>
  {/if}
