<script lang="ts">
  ...
  $effect(() => {
    ...
    if (collection) {
      ...
      copyNewName = `${collection.label} (cópia)`;
  ...
  async function handleSave() {
    ...
    const duplicate = ctx.collections.find((c) => c.label === trimmedLabel && c.id !== collection?.id);
    if (duplicate) {
      ...
      <div class="space-y-6 p-4">
        {#if showCopyMode && collection}
          <div class="space-y-2">
            ...
            <p class="rounded-lg border border-app-border bg-app-surface-muted px-3 py-2 text-sm">{collection.label}</p>
          </div>
          <div class="space-y-2">
          ...
          <label class="flex items-center justify-between gap-3 text-sm">
            <span class="text-app-muted">Incluir anúncios</span>
            ...
            {#if isEditing && collection}
              ...
              {#if showDeleteConfirm}
                <div class="space-y-2 rounded-lg border border-destructive/30 bg-destructive/10 p-3">
                  <p class="text-sm">Excluir "{collection.label}"?</p>
                  <div class="flex gap-2">
