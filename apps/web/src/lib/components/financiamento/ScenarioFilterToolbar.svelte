<script lang="ts">
  import { DollarSign, House } from "@lucide/svelte";
  import {
    PERCENTAGE_OPTIONS,
  ...
  const priceOptions = $derived(
    PERCENTAGE_OPTIONS.map((option) => ({
      ...option,
      valor: Math.round(params.valorImovelSelecionado * option.value)
    }))
  );
  ...
  const propertyOptions = $derived(
    PERCENTAGE_OPTIONS.map((option) => ({
      ...option,
      valor: Math.round(params.valorApartamentoSelecionado * option.value)
    }))
  );
...
<div class={LISTINGS_TOOLBAR_CLASS}>
  <div class={cn(LISTINGS_TOOLBAR_INNER_CLASS, "gap-1.5")}>
