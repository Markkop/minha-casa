<script lang="ts">
  ...
  const displayTitle = $derived(ctx.getPropertyListDisplayTitle(listing));
...
<div class="text-sm min-w-[200px] max-w-[280px]">
  ...
  <p class="text-gray-600 text-xs mb-2">{listing.address}</p>
  {#if customLoc}
    <p class="text-xs text-blue-600 mb-2 font-medium">Localização personalizada</p>
  {/if}
  <div class="space-y-1">
    {#if listing.price !== null}
      <p class="text-base font-semibold text-green-600">{formatCurrency(listing.price)}</p>
    {/if}
    {#if listing.totalAreaM2}
