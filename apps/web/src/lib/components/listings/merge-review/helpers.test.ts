...
import {
  ...
  visibleMergeFields
} from "$lib/components/listings/merge-review/helpers";
import type { MergeReviewField, MergeReviewSession } from "$lib/components/listings/merge-review/types";
...
describe("merge review selection helpers", () => {
  it("selects fields and selectable gallery items by default", () => {
    expect(createDefaultMergeSelection(readySession())).toEqual({
      fieldPaths: ["price", "title", "features.pool"],
      fieldValues: { title: "Importado" },
  ...
  it("limits the default selection to suggested fields when suggestions exist", () => {
    const session: MergeReviewSession = {
      ...readySession(),
      suggestions: [
        { path: "price", suggestedValue: 530_000, note: "Preço mais recente." },
        { path: "title", suggestedValue: "Título sugerido", note: null },
        { path: "naoExiste", suggestedValue: "x", note: null }
      ]
    };

    expect(createDefaultMergeSelection(session)).toEqual({
      fieldPaths: ["price", "title"],
      fieldValues: { title: "Título sugerido", price: 530_000 },
      imageRefs: ["existing:0", "new:image-1"]
    });
  });
  ...
  it("maps only suggestions that reference diff fields", () => {
    const session: MergeReviewSession = {
      ...readySession(),
      suggestions: [
        { path: "price", suggestedValue: 530_000 },
        { path: "naoExiste", suggestedValue: "x" }
      ]
    };

    expect([...mergeSuggestionMap(session).keys()]).toEqual(["price"]);
  ...
  it("shows only suggested fields by default and all fields on demand", () => {
    const session: MergeReviewSession = {
      ...readySession(),
      suggestions: [{ path: "price", suggestedValue: 530_000 }]
    };

    expect(visibleMergeFields(session, false).map((field) => field.path)).toEqual(["price"]);
  ...
  it("adds and removes selection items without mutating the source", () => {
    ...
    expect(setMergeSelectionItem(initial, "neighborhood", true)).toEqual(["price", "neighborhood"]);
    expect(setMergeSelectionItem(initial, "price", true)).toEqual(["price"]);
    expect(setMergeSelectionItem(initial, "price", false)).toEqual([]);
    expect(initial).toEqual(["price"]);
  });

  it("requires at least one selected field or image", () => {
    expect(hasMergeSelection({ fieldPaths: [], fieldValues: {}, imageRefs: [] })).toBe(false);
    expect(hasMergeSelection({ fieldPaths: ["price"], fieldValues: {}, imageRefs: [] })).toBe(
      true
    );
...
describe("merge review formatting helpers", () => {
  ...
  it("keeps field groups in first-seen order", () => {
    const fields: MergeReviewField[] = [
      { path: "price", label: "Preço", group: "Valores", currentValue: 1, incomingValue: 2 },
      { path: "neighborhood", label: "Bairro", group: "Localização", currentValue: "A", incomingValue: "B" },
    ...
    expect(groups.get("Valores")?.map((field) => field.path)).toEqual(["price", "condominio"]);
  });
});
