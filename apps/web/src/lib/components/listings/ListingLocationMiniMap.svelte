<script lang="ts">
  ...
  import { resolveListingLocation, type ResolvedListingLocation } from "$lib/listings/listing-location";
  import {
    calculatePricePerM2,
  ...
  import { MAP_EMBED_PANEL_CLASS } from "$lib/listings/listings-panel-layout";
  import { mapPriceColors } from "$lib/theme/colors";
  import { buildGoogleMiniMarkerContent } from "$lib/listings/map-google-markers";
  import {
    createLeafletMiniMarkerIcon,
    LEAFLET_TILE_ATTRIBUTION,
    LEAFLET_TILE_URL
  } from "$lib/listings/map-leaflet-markers";
    ...
    void (async () => {
      if (google) {
        ...
        try {
          ...
          const pricePerM2 = calculatePricePerM2(listing.price, listing.totalAreaM2) ?? listing.pricePerM2;
          const color = getMarkerColor(pricePerM2, pricePerM2 ?? 0, pricePerM2 ?? 0);
      ...
      const pricePerM2 = calculatePricePerM2(listing.price, listing.totalAreaM2) ?? listing.pricePerM2;
      const color = getMarkerColor(pricePerM2, pricePerM2 ?? 0, pricePerM2 ?? 0) || mapPriceColors.unknown;
...
{#if loading}
  <div class={cn(containerClass, loadingShellClass)} aria-label={`Carregando mapa de ${listing.title}`}>
