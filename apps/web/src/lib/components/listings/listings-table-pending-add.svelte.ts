...
import { checkDuplicateCandidates } from "$lib/listings/check-duplicate";
import { resolveMergeGallery } from "$lib/components/listings/merge-review";
import { buildParseRequestFromFile } from "$lib/listings/parse-input";
import type { ParseRequest } from "$lib/listings/parse-input-types";
import { assertPublicListingUrl, normalizeListingUrlInput } from "$lib/listings/listing-url";
import { formatApiError } from "$lib/api/error-message";
import { ApiError } from "$lib/api/client";
import {
  LISTING_IMPORT_QUEUE_EVENT,
  type ListingImportQueueDetail
} from "$lib/listings/listing-import-queue";
import {
  createPendingId,
  type PendingAddRow
...
} from "$lib/listings/clipboard-errors";
...
export function createListingsTablePendingAdd(getCtx: () => CollectionsContextValue) {
  ...
  async function retryMerge() {
    ...
    const importedData = mergeSession.importedData as unknown as ListingData;
