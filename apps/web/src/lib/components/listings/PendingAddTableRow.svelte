<script lang="ts">
  ...
  import type { ListingsTableColumn } from "$lib/components/listings/listings-table-shared";
  import { LISTING_THUMB_SIZE_CLASS } from "$lib/components/listings/listings-table-shared";
  ...
  import { formatCurrency } from "$lib/listings/map-shared";
      ...
      {:else if row.status === "skipped"}
        <div class="flex min-w-0 flex-col gap-1.5 whitespace-normal py-1">
          ...
          <p class="text-xs text-app-muted">
            Nada para atualizar.
            <button type="button" class="cursor-pointer font-medium text-app-success hover:underline" onclick={() => onConfirmDuplicate(row.id)}>
              Salvar mesmo assim
            </button>
            {" ou "}
  ...
  {#if visibleColumns.price}
    <td class={LISTING_TABLE_DATA_CELL_CENTER_CLASS}>
      {#if isDuplicatePreview && parsedPreview}
        <div class={cn(metricMutedClass, "text-center")}>{formatCurrency(parsedPreview.price ?? null)}</div>
      {:else}
    ...
    <td class={LISTING_TABLE_DATA_CELL_CENTER_CLASS}>
      {#if isDuplicatePreview && parsedPreview}
        <div class={cn(metricMutedClass, "flex justify-center")}>
          <AreaM2Stack
            total={parsedPreview.totalAreaM2 ?? null}
            privado={parsedPreview.privateAreaM2 ?? null}
            propertyType={parsedPreview.propertyType}
  ...
  {#if visibleColumns.value}
    <td class={LISTING_TABLE_DATA_CELL_CENTER_CLASS}>
      {#if isDuplicatePreview && parsedPreview}
        <div class={cn(metricMutedClass, "flex justify-center")}>
          <PricePerM2Stack
            total={calculatePricePerM2(parsedPreview.price ?? null, parsedPreview.totalAreaM2 ?? null)}
            privado={calculatePrivateAreaPricePerM2(parsedPreview.price ?? null, parsedPreview.privateAreaM2 ?? null)}
            propertyType={parsedPreview.propertyType}
            activeVariant={activeMetricVariant}
            enabledVariants={enabledMetricVariants}
            align="center"
