...
export const LISTING_STAGE_OPTIONS: { value: ListingStage; label: string; className: string }[] = [
  ...
  { value: "discarding", label: "Descartando", className: "border-orange-500/30 bg-orange-500/10 text-orange-700" },
  { value: "discarded", label: "Descartado", className: "border-destructive/30 bg-destructive/10 text-destructive" },
  { value: "sold", label: "Vendido", className: "border-slate-500/30 bg-slate-500/10 text-slate-600" }
];

const LISTING_STAGE_VALUES = new Set<ListingStage>(LISTING_STAGE_OPTIONS.map((option) => option.value));
const STRIKETHROUGH_STAGES = new Set<ListingStage>(["discarded", "sold"]);

export function isStrikethroughStage(stage: ListingStage): boolean {
  return STRIKETHROUGH_STAGES.has(stage);
}

export const LISTING_THUMB_SIZE_CLASS = "h-20 w-20 flex-shrink-0 aspect-square";
export type PropertyTypeValue = "house" | "apartment" | null;

export const PROPERTY_TYPE_OPTIONS = [
  { value: null as PropertyTypeValue, label: "Não definido", Icon: Home },
  { value: "house" as const, label: "Casa", Icon: Home },
  { value: "apartment" as const, label: "Apartamento", Icon: Building }
];

export function normalizePropertyType(value: Property["propertyType"]): PropertyTypeValue {
  if (value === "house" || value === "apartment") return value;
  return null;
}

export function getPropertyTypeOption(value: Property["propertyType"]) {
  const normalized = normalizePropertyType(value);
  return PROPERTY_TYPE_OPTIONS.find((option) => option.value === normalized) ?? PROPERTY_TYPE_OPTIONS[0];
}

export function getListingStage(property: Pick<Property, "stage" | "strikethrough" | "visited">): ListingStage {
  if (property.stage && LISTING_STAGE_VALUES.has(property.stage as ListingStage)) {
    return property.stage as ListingStage;
  }
  if (property.strikethrough) return "discarded";
  if (property.visited) return "visited";
  return "analyzing";
}

export function getListingStageOption(stage: ListingStage) {
  return LISTING_STAGE_OPTIONS.find((option) => option.value === stage) ?? LISTING_STAGE_OPTIONS[0];
}

export const STAGE_TRIGGER_WIDTH = "w-[128px]";
...
/** Aligned summary grid: price/areas | stage/features */
...
/** Native stage chip: matches Radix select trigger spacing and chevron. */
export const LISTING_STAGE_SELECT_APPEARANCE_CLASS =
...
const LEGACY_STAGE_COLUMN_KEY = "etapa";
...
export function normalizeVisibleColumns(value: unknown): Record<ListingsTableColumn, boolean> {
  ...
  const raw = value as Partial<
    Record<ListingsTableColumn | typeof LEGACY_STATUS_COLUMN_KEY | typeof LEGACY_STAGE_COLUMN_KEY, unknown>
  >;
  return LISTINGS_TABLE_COLUMNS.reduce(
    (acc, column) => {
      const storedValue =
        column.id === "stage" && typeof raw.stage !== "boolean"
          ? typeof raw[LEGACY_STAGE_COLUMN_KEY] === "boolean"
            ? raw[LEGACY_STAGE_COLUMN_KEY]
