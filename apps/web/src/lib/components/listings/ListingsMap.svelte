<script lang="ts">
  ...
  import { geocodeAddresses, clearCacheForAddresses } from "$lib/listings/geocoding";
  import { buildListingGeocodeQuery } from "$lib/listings/listing-location";
  ...
  import MapLocationPicker from "$lib/components/listings/MapLocationPicker.svelte";
  import ToolbarAnchoredPopover from "$lib/components/listings/ToolbarAnchoredPopover.svelte";
  import LeafletMapView from "$lib/components/listings/LeafletMapView.svelte";
  import GoogleMapsView from "$lib/components/listings/GoogleMapsView.svelte";
  import ListingsThreeMapView from "$lib/components/listings/ListingsThreeMapView.svelte";

  let {
    listings,
    collectionLabel = "Coleção atual",
    getListingTitle = (listing: Property) => listing.title || listing.address || "Imóvel"
  } = $props<{
    listings: Property[];
    ...
    void (async () => {
      ...
      for (const listing of listings) {
        if (listing.customLat != null && listing.customLng != null) {
          withCustom.push({
            listing,
            location: { lat: listing.customLat, lng: listing.customLng, displayName: listing.address }
          });
        } else {
      ...
      if (toGeocode.length > 0) {
        ...
        const addressInputs = toGeocode
          .map((l) => {
            ...
            return query ? { address: query, city: l.city } : null;
          })
          .filter((input): input is { address: string; city: string | null | undefined } => input !== null);
  ...
  $effect(() => {
    if (!mounted || isLoading || geocodedListings.length === 0 || boundsApplied || hasUserMapViewportFeature()) return;
  ...
  const minMaxPreco = $derived.by(() => {
    const prices = geocodedListings
      .map((gl) => calculatePricePerM2(gl.listing.price, gl.listing.totalAreaM2))
  ...
  const missingListings = $derived.by(() => {
    ...
    return listings.filter((listing: Property) => !ids.has(listing.id));
  });
  ...
  function handleRedoGeocoding() {
    const addressInputs: { address: string; city: string | null | undefined }[] = [];
    for (const listing of listings) {
      const query = buildListingGeocodeQuery(listing);
      if (query) addressInputs.push({ address: query, city: listing.city });
    }
            ...
            >
              ...
              <ul class="list-inside list-disc space-y-1 text-xs">
                {#each missingListings as listing (listing.id)}
                  <li>{listing.address || listing.title}</li>
                {/each}
              </ul>
            </ToolbarAnchoredPopover>
          {:else}
