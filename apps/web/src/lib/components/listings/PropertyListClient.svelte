<script lang="ts">
  ...
  import { getDefaultFirstCollectionName } from "$lib/listings/default-first-collection-name";
  import { LISTINGS_PAGE_CLASS } from "$lib/listings/listings-panel-layout";
  import { WORKSPACE_CONTENT_CLASS } from "$lib/workspace-chrome";
  import { getCollectionsContext } from "$lib/collections-context.svelte";
  import { toListingData } from "$lib/listings/types";
  import type { ListingData } from "$lib/workspace/client";
  import PropertyListQuerySync from "$lib/components/listings/PropertyListQuerySync.svelte";
  import ListingsTable from "$lib/components/listings/ListingsTable.svelte";
  ...
  async function loadShareFromToken(token: string) {
    ...
    try {
      ...
      shareData = {
        ...
        listings: loaded.listings.map((listing) => toListingData(listing) as ListingData)
      };
  ...
  <div class="min-h-[calc(100vh-var(--nav-height,2.75rem))] bg-app-bg text-app-fg">
    ...
    <main class={LISTINGS_PAGE_CLASS}>
      ...
      <ListingsMap
        ...
        collectionLabel={ctx.activeCollection?.name ?? "Coleção atual"}
