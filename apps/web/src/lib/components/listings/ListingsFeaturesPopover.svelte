<script lang="ts">
  ...
  let {
    ...
    onChange: (catalog: ListingFeatureOption[]) => void | Promise<void>;
    saving?: boolean;
  } = $props();
  ...
  const sortedCatalog = $derived(sortFeatureCatalog(catalog));
  ...
  function addCustomFeature() {
    ...
    const baseKey = slugifyFeatureKey(label);
    const key = ensureUniqueFeatureKey(baseKey, catalog);
