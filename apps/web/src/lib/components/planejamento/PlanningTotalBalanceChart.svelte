<script lang="ts">
  ...
  const legendNote =
    "Saldo acumulado após receitas e despesas · linha horizontal tracejada: saldo zero";

  const visibleMonths = $derived(visiblePlanningMonths(months, viewport));
  const yAxis = $derived(
    buildPlanningYAxisScale(visibleMonths.map((month) => month.closingBalance))
  );
    ...
    return [
      {
        ...
        y: yForLedgerValue(hoveredMonth.closingBalance, yAxis, height, padding),
  ...
  const breakdownMarkerPoint = $derived.by(() => {
    ...
    const svgY = yForLedgerValue(hoveredMonth.closingBalance, yAxis, height, padding);
...
<CollapsibleChartPanel title="Saldo total ao longo do tempo" empty={months.length === 0}>
  <div bind:this={chartContainer} class="relative w-full" style:min-height="{height}px">
    ...
    <svg
      ...
      role="img"
      aria-label="Gráfico de saldo total do planejamento"
    >
      <TimelineChartAxes
        {yTicks}
        {xMonthGrid}
    ...
    {#if hoveredMonth}
      ...
      >
        ...
        <dl class="grid grid-cols-[auto_1fr] gap-x-2 gap-y-0.5 text-app-muted">
          ...
          <dt>Saldo inicial</dt>
          <dd class="font-mono text-app-fg">{formatCurrency(hoveredMonth.openingBalance)}</dd>
          <dt>Receitas</dt>
          ...
          {#if hoveredMonth.totalDebt > 0}
            <dt>Dívida</dt>
            <dd class="font-mono">{formatCurrency(hoveredMonth.totalDebt)}</dd>
          {/if}
          <dt class="font-bold text-app-accent">Saldo acumulado</dt>
          <dd
            class={cn(
              "font-mono font-bold",
              hoveredMonth.closingBalance < 0 ? "text-salmon" : "text-app-accent"
            )}
          >
            {formatCurrency(hoveredMonth.closingBalance)}
          </dd>
        </dl>
      </ChartHoverBreakdownPanel>
    {/if}
