...
export interface ListingsPropertyDisplayPrefs {
  showAddress: boolean
  /** Quartos, bathrooms, parkingSpots e floor na listagem. */
...
export function shouldShowPropertyTypeFilters(
  listings: { propertyType?: string | null }[]
): boolean {
  const hasCasa = listings.some((listing) => listing.propertyType === "house")
  const hasApartamento = listings.some((listing) => listing.propertyType === "apartment")
