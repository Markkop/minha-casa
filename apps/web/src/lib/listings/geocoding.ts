...
function cacheKeyFor(input: GeocodeAddressInput | string, options?: GeocodeQueryOptions): string {
  ...
  const city = typeof input === "string" ? options?.city : input.city ?? options?.city
  return city?.trim() ? `${address}::${city.trim()}` : address
}
...
/**
 * Clear cache for specific addresses (supports city-scoped keys).
 */
