...
} from "$lib/listings/clipboard-listing-detection";
...
describe("detectClipboardListingContent", () => {
  it("detects known portal URLs", () => {
    for (const domain of KNOWN_PORTAL_DOMAINS.slice(0, 5)) {
      const result = detectClipboardListingContent(`https://www.${domain}/property/123`);
      expect(result).toEqual({ kind: "url", preview: `https://www.${domain}/property/123` });
    }
  });
  ...
  it("detects listing-like text with multiple keywords", () => {
    const text =
      "Apartamento 2 bedrooms, 1 suíte, 72 m², condomínio R$ 450, aluguel R$ 2.800";
