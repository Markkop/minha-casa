...
<main class="grid min-h-screen place-items-center bg-app-bg px-4">
  <form class="w-full max-w-md rounded-md border border-app-border bg-app-surface p-6 shadow-sm" onsubmit={(event) => { event.preventDefault(); void login(); }}>
    ...
    <input id="password" bind:value={password} class="mt-2 h-10 w-full rounded-md border border-app-border bg-white px-3" type="password" autocomplete="current-password" required />

    {#if error}
      <p class="mt-4 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>
    {/if}

    <Button class="mt-6 w-full" type="submit" disabled={loading}>{loading ? "Entrando..." : "Entrar"}</Button>
    <Button class="mt-3 w-full" variant="secondary" onclick={google}>Entrar com Google</Button>
    <p class="mt-5 text-center text-sm text-app-muted">Ainda nao tem conta? <a class="font-medium text-app-fg" href="/signup">Criar conta</a></p>
  </form>
</main>
