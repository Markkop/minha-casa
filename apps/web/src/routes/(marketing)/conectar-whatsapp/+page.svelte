<script lang="ts">
  ...
  const pendingKey = "minha-casa:pending-wa-link-code";
  ...
  const redirectPath = $derived(code ? `/conectar-whatsapp?wa=${encodeURIComponent(code)}` : "/conectar-whatsapp");

  onMount(async () => {
    const params = new URLSearchParams(window.location.search);
    const urlCode = params.get("wa")?.trim() ?? "";
  ...
  async function loadStatus() {
    try {
      status = await workspaceApi.fetchWhatsAppStatus();
    } catch {
      status = { linked: false };
    }
  }
  ...
  async function connect(linkCode = manualCode) {
    ...
    try {
      await workspaceApi.linkWhatsApp(trimmed);
  ...
  <section class="w-full max-w-md rounded-md border border-app-border bg-app-surface p-6 shadow-sm">
    ...
    {:else if !isAuthenticated && code}
      <p class="mt-3 text-sm leading-6 text-app-muted">
        Entre ou crie sua conta para vincular o WhatsApp. O codigo
